...
# At the top of transform.py
def _map_with_default(mapping, key):
...
class TargetTransform(BaseTransform):
    ...
    def _set_preprocessing_mapper(self, classification_preprocessing_dict):
        d = dict(classification_preprocessing_dict)
        self.preprocessing_mapper = np.vectorize(partial(_map_with_default, d))
    ...
    def _set_mapper(self, classification_dict):
        ...
        d[65] = 65
        self.mapper = np.vectorize(partial(_map_with_default, d))
    ...
    def __call__(self, data):
        ...
        if points_to_drop.sum() > 0:
            ...
            if "idx_in_original_cloud" in data:
                data.idx_in_original_cloud = data.idx_in_original_cloud[
                    points_to_keep.numpy()
                ]
